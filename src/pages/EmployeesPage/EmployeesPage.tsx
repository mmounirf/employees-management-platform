import { useTheme } from '@emotion/react'
import { Button, Center, Container, Grid, Pagination, Paper, Text, useMantineColorScheme } from '@mantine/core'
import { IconPlus } from '@tabler/icons-react'
import { useEffect, useState } from 'react'
import { useNavigate } from 'react-router-dom'

import { useGetEmployeesQuery, useLazyGetEmployeesQuery } from '../../api'
import { EmptyView } from '../../components'

import { PageHeader } from './../../components/PageHeader/PageHeader'
import { EmployeeCard } from './components/EmployeeCard'
import { EmployeeSearch } from './components/EmployeeSearch'
import { LoadingSkeleton } from './components/LoadingSkeleton'

const extractSearchValue = (query: string) => {
  try {
    const jsonQuery = JSON.parse(query) as { [key: string]: { $regex: string } }
    return Object.values(jsonQuery)[0].$regex
  } catch (error) {
    return undefined
  }
}

export const EmployeesPage = () => {
  const navigate = useNavigate()
  const { colorScheme } = useMantineColorScheme()
  const [limit, setLimit] = useState(10)
  const theme = useTheme()

  const [activePage, setPage] = useState(1)
  const [query, setQuery] = useState('')
  const [search, { data: searchResponse, isFetching: isSearching }] = useLazyGetEmployeesQuery()
  const { data: response, isFetching } = useGetEmployeesQuery({ limit, page: activePage })
  const dataToRender = query.length > 2 ? searchResponse : response

  const dark = colorScheme === 'dark'

  useEffect(() => {
    if (query.length) {
      void search({ s: query })
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [query])

  return (
    <Container fluid p={0}>
      <Paper
        sx={{
          position: 'sticky',
          top: 70,
          zIndex: 100,
          background: dark ? theme.colors.dark[8] : theme.colors.gray[0],
        }}
      >
        <PageHeader
          title="Employees"
          subtitle="List, search and filter your employees' records"
          actionButton={
            <Button variant="light" leftIcon={<IconPlus />} onClick={() => navigate('/employees/new')}>
              Add new
            </Button>
          }
        />
        <EmployeeSearch loading={isSearching} onSearch={setQuery} onLimitChange={setLimit} />
      </Paper>

      <Container fluid p="md">
        <Grid mt="md">
          {isFetching || isSearching ? (
            <LoadingSkeleton />
          ) : dataToRender?.data.data.length ? (
            dataToRender?.data.data.map((employeeData) => (
              <Grid.Col md={6} lg={3} key={employeeData.id}>
                <EmployeeCard {...employeeData} />
              </Grid.Col>
            ))
          ) : (
            <Center w="100%" my="xl">
              <EmptyView
                label={
                  extractSearchValue(query) ? (
                    <Text>
                      No empolyees records found matching your search input <strong>{extractSearchValue(query)}</strong>
                    </Text>
                  ) : (
                    <Text>Employees database is empty</Text>
                  )
                }
              />
            </Center>
          )}
        </Grid>

        <Paper
          sx={{ position: 'sticky', bottom: 0, background: dark ? theme.colors.dark[8] : theme.colors.gray[0] }}
          p="md"
        >
          <Center>
            <Pagination total={dataToRender?.pageCount ?? 0} onChange={setPage} withEdges withControls />
          </Center>
        </Paper>
      </Container>
    </Container>
  )
}
