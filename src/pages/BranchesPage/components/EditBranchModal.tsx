...
import { useEditBranchMutation, type EditBranchPayload } from '../../../api'
...
export const EditBranchModal = ({ data }: EditBranchModalProps) => {
  ...
  const onSave = async () => {
    ...
    const response = await editBranch(form.values).unwrap()

    if (response) {
      showNotification({
        message: `Branch ${form.values.name.en} has been updated`,
        icon: <IconCheck />,
        color: 'green',
      })
      closeAllModals()
